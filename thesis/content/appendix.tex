...
Table~\ref{tab:hadron-scattering} lists all adjustable parameters as well as some meson lifetimes relevant for cooling.
In the context of this approach, the variable $M$ is related to $H = \pi (\hbar c / M)^2$ and
$s_h = (m_h + m_p + M)^2$ as an effective mass in units of energy.

\begin{equation*}
	\mathscr{R} = \frac{\sigma_{\text{el}\kern+0.5pt} (s)}{\sigma_{\text{tot}\kern+0.5pt} (s)} =
	\mathscr{A} \tanh \bigl( \kern+1.0pt \gamma_1 \kern-1.0pt - \gamma_2 \ln (s) + \gamma_3 \ln^2 (s) \bigr)
\end{equation*}

\input{content/tables/hadron-ratio.tex}

$\sigma_\text{el} + \sigma_\text{inel} = \sigma_\text{tot}$ $\sigma_\text{el} = \mathscr{R} \sigma_\text{inel}$
$\sigma_\text{inel} = (1 - \mathscr{R}) \sigma_\text{tot}$
