...
\end{align*}
where both mass quantities $s_h$ and $m_h$ are listed in table \ref{tab:charm-hadrons} for reference. The assumption of three
body decays such as $D^+ \kern-2.5pt \rightarrow \kern+1.0pt \overline{\kern-1.5pt K \kern+1.0pt}\kern+1.0pt^0 e^+ \nu_e$ can
be justified by consulting \cite{pdg} for information on relevant particles and comparing branching ratios, which indicate that
purely leptonic modes are strongly suppressed. Hadronic channels like $D^+ \kern-2.5pt \rightarrow \pi^+ \pi^0$ or
$D^+ \kern-2.5pt \rightarrow K^- \pi^+ \pi^+$ are either very improbable as well or occur at significant rates but do not
contribute many high energy neutrinos due to pions and kaons being subject to further cooling before decaying to leptons.
...
\begin{equation*}
	F \kern+1.5pt ( x_\pi, E_p ) = 4\alpha B x_\pi^{\alpha - 1} \left( \frac{1 - x_\pi^\alpha}{1 + rx_\pi^\alpha (1 - x_\pi^\alpha \kern+1.0pt )}
	\right)^{\kern-1.0pt 4} \left( \frac{1}{1 - x_\pi^\alpha} + \frac{r (1 - 2x_\pi^\alpha \kern+1.0pt )}{1 + rx_\pi^\alpha
	(1 - x_\pi^\alpha \kern+1.0pt )} \right) \left( 1 - \frac{m_0}{x_\pi E_p} \right)^{\kern-1.0pt 1/2}
\end{equation*}
