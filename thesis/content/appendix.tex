...
\begin{equation*}
	P(t) = 1 - \kern+1.0pt \exp \kern-2.0pt
	\raisebox{0.25ex}{\(\Bigl( \raisebox{-0.25ex}{\(-\frac{\raisebox{-0.5ex}{$t$}}{\raisebox{1.0ex}{$\tau \kern+1.0pt$}}\)}
	\kern+0.5pt \Bigr)\)}
\end{equation*}
...
lose energy in every collision, which is the same as reducing temperature from a thermodynamics perspective.



Substituting
like $t / \tau \kern+0.5pt \rightarrow \lambda_\text{cool} \kern+0.5pt / \lambda_\text{dec}$ or equivalently
$t / \tau \kern+0.5pt \rightarrow \kern+0.3pt t_\text{cool} \kern+0.5pt / t_\text{dec}$ accounts for cooling effects, provided
$\lambda_\text{dec} \ll d$ holds with $d$ as the size of a homogenous target field to ensure decays happen exclusively inside
this region. The ratio $t_\text{dec} \kern+0.5pt / t_\text{cool}$ is obtained by rearranging variables, instead of
