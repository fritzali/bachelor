...
\end{equation*}
as the integrated spectrum. Accordingly, one finds
\begin{equation*}
...
\end{equation*}
for the differential spectrum where
\begin{equation*}
	\alpha = \frac{\ln(\varsigma)}{\ln(\eta)} - 1
\end{equation*}
defines the spectral index. From $\varsigma < 1$ and $\eta > 1$ follows $\alpha + 1 < 0$ as a general constraint.
