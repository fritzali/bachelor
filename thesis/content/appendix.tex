...
Reference \cite{Fagundes_2013} tests the asymptotic rise $\sigma(s) \propto \ln^2(s)$ derived in \cite{Froissart_1961} as a
theoretical upper bound from the optical theorem and concludes that it is somewhat exceeded. Additionally, a very high energy
ratio $\mathscr{A} = 1/3$ due to diffraction as opposed to the classical black disc limit $\mathscr{A} = 1/2 \kern+0.2pt$ is
suggested. Unfortunately, parameters with $\mathscr{A} = 1/3$ are omitted in \cite{Fagundes_2013} so that for this work, all
calculations requiring $\mathscr{R}$ to find $\sigma_{\text{inel}\kern+0.5pt}(s)$ use the function \eqref{eqn:ratio} defined
by $\mathscr{A} = 1/2 \kern+0.2pt$ instead.



\subsection*{Production}

For the production of charm quarks in proton air collisions, reference \cite{Goncalves_2007} gives
\begin{equation*}
...
\end{equation*}
as a parametrization for the differential cross section with components
\begin{align*}
	a = a_1 \kern-1.0pt - a_2 \ln\bigl(E_p\bigr) && b = b_1 \kern-1.0pt - b_2 \ln\bigl(E_p\bigr) &&
	n = n_1 \kern-1.0pt - n_2 \ln\bigl(E_p\bigr)
\end{align*}
for which table \ref{tab:charm-production} lists all necessary parameters.

Feynman scaling variable $x_F \kern+1.0pt = p_c \kern+1.5pt / \kern-0.5pt p_p$
