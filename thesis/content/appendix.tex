...
fractions of \qty{99.99}{\percent} for $h = \smash{\pi^+}$ and \qty{63.56}{\percent} for $h = \smash{K^+}$ given in the \cite{pdg}
...
\cite{Carpio_2020} suggest that this is particularly relevant for pions. Muonic three body decays 
...
computations and is omitted in service of restricting the present work to a managable scope.

The remaining two body decays of ultrarelativistic hadrons $h$ to leptons $l \kern+1.0pt$ obey a distribution
\begin{equation*}
	F_{\kern+1.0pt h \kern+0.2pt \rightarrow \nu \kern+0.5pt} \bigl( E_l \kern+0.8pt, \kern-0.2pt E_h \bigr) =
	E_h^{\kern+1.0pt -1} \bigl( 1 \kern-0.3pt - \lambda_h \bigr)^{-1}
\end{equation*}
