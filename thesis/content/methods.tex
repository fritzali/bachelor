...
Borrowing from \cite{Bhattacharya_2015} among others, linear scaling with nucleon number is assumed, giving
\begin{equation*}
...
particle, probabilities for color neutral-{\kern+0.2pt}states existing in individual partons are given by the appropriate \abbrev{FF} instead.
The partons described here are either quarks or gluons, which sections \ref{sub:interactions} and \ref{sub:hadrons} describe
more closely. By fitting to existing data or perturbative results, models can extrapolate to low momentum fractions that have
not yet been probed experimentally.
\newpage
Such a procedure has lead \cite{Kniehl_2006} to obtain
\begin{equation}
...
\end{equation*}
with $\mathscr{S}$ as an arbitrary unitless normalization factor that does not need to be specified further, as this work is
interested in relative neutrino contributions instead of absolute predictions.
