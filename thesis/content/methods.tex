...
that particles $b$ are produced in a singular interaction, one can solve the folding integral
\begin{equation}
	\frac{dN_b}{\raisebox{0.5ex}{$dE_b$}} (E_b) \kern+0.5pt = \kern-1.5pt\int_{\check{E}_a}^{\hat{E}_a} dE_a
	\kern+0.5pt \frac{dN_a}{\raisebox{0.5ex}{$dE_a$}} (E_a) \kern+1.5pt F_{a \rightarrow b} (E_b, E_a)
	\label{eqn:folding}
\end{equation}
...
To implement and compute the following parametrizations, the \emph{Python} programming language and its package \emph{Numpy}
are used. Results are presented visually with the \emph{Matplotlib} library.\footnote{$\,$In service of reproducability, all
