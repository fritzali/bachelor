...
decay times resulting in vanishing cooling factors \eqref{eqn:cooling} at higher energies, curves are shifted according to the proton
spectrum formulation \eqref{eqn:delta} with a delta function, reproducing the slight offsets in charmed hadron contribution.
...
agreement, with distribution shapes that are roughly similar in either case. Because of differences in the approaches, some deviation is
...
energy hadrons are available for neutrino production, resulting in a depression of otherwise possible peak values. Comparing to
\cite{Carpio_2020} points to figure \ref{fig:magnetar-flux-without} displaying both of these features. Smaller injection energies
...
better matches than figures \ref{fig:magnetar-flux-with} and \ref{fig:magnetar-fluence-with} with $\mathscr{O}$ to \cite{Carpio_2020}.
Though this problem should certainly be investigated further, the following discussion treats the results without optical
depth as valid, as they better agree with prior expectations as well as \cite{Carpio_2020}.
