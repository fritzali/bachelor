...
kaons. As stated above, this is at least partly attributable to the larger maximum proton energy given in \cite{Carpio_2020}.

Continuing with the results from an AGN accretion disk, there exists no temporal dimension, because the model
assumes a constant neutrino spectrum as a first approximation. Depicted below in figure \ref{fig:nucleus-charm-comparison}
...
This agrees well with findings for the magnetar setting, as $\smash{D^0}$ and $\smash{D^+} \kern-0.5pt$ dominate over $\smash{D^+_s}$
and $\smash{\Lambda^{\kern-0.5pt +}_{\kern+0.5pt c}}$ components at all energies. Due to its larger branching fraction, decay of
$\smash{D^+} \kern-0.5pt$ contributes slightly more than $\smash{D^0}$ up to $E_\nu = \kern-0.5pt \qty{e9}{\giga\electronvolt}$
despite having a smaller production cross section \cite{lhc}. At higher energies, its longer mean lifetime given in table
\ref{tab:charm-hadrons} leads to $\smash{D^+} \kern-0.5pt$ being suppressed by cooling, allowing $\smash{D^0}$ to become
the dominant charm contributor.
